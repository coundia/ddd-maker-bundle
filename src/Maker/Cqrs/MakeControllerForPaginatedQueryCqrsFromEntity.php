...
class MakeControllerForPaginatedQueryCqrsFromEntity extends BaseMaker{
	...
	public function configureCommand(
		...
		$command->addArgument('query', InputArgument::OPTIONAL, 'Name of the query action',"FindAll");
	...
	public function generate(
		...
		$controllers = [
			[
				'template'  => Common::RESOURCE_DIR . '/Aggregate/Presentation/Controller/CqrsEntityPaginatedListController.tpl.php',
				'variables' => [
					'namespace'         => $controllerNamespace,
					'class_name' => $action.$entityShort . 'Controller',
					'route_path'        => '/api/' . $api_version . strtolower($entityShort) . 's/list',
					...
					'query_name' => $applicationNamespace.'\\Query\\'.$action.$entityShort.'Paginated'.'Query',
					'parameter' => "",
		...
		(new MakeTestsPaginatedQueryControllersFromEntity($this->baseDir))->generate(
			$input,
			$io,
			$generator
		);
