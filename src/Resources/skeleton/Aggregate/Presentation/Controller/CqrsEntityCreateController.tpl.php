...
private <?= $command_interface ?> $command_bus,
...
<?php foreach ($attributes as $attribute): ?>
	...
	<?php if ($attribute->isEntity()): ?><?php $caster = '(string)';$getter .= $attribute->getName();  ?>
		<?= $attribute->getName() ?>: $this->mapper<?= ucfirst(
			$attribute->getName()
		) ?>->toEntity($this->finder<?= ucfirst(
			$attribute->getName()
		) ?>->find(\App\<?= $context ?>\Domain\ValueObject\<?= ucfirst($attribute->getName()) ?>Id::create(<?= $caster ?> $command_model-><?= lcfirst($attribute->getName()) ?>))),
		<?php continue;  endif; ?>
	<?php if ($attribute->isValueObject()): ?>
		<?= $attribute->getName() ?>: $command_model-><?= $attribute->getName() ?>,
		<?php continue; endif; ?>
	<?= $attribute->getName() ?>: $command_model-><?= $attribute->getName() ?>,
<?php endforeach; ?>
);
